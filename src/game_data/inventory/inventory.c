...
inventory_t* create_empty_inventory(const memory_pool_t* pool, int pre_length) {
    ...
    RETURN_WHEN_TRUE(pre_length < 0, NULL, "Inventory", "In `create_empty_inventory` max_items is negative")
    ...
    if (pre_length == 0) {
        ...
        inventory->items = (item_t**) memory_pool_alloc(pool, sizeof(item_t*) * pre_length);
        RETURN_WHEN_NULL(inventory->items, NULL, "Inventory", "In `create_empty_inventory` failed to allocate memory for items")
    }
    ...
    inventory->max_items = pre_length;
    return inventory;
}
